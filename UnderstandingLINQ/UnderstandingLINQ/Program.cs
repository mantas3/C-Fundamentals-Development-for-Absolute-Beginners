...
namespace UnderstandingLINQ
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var _bmws = myCars.Where(car => car.Make == "BMW").Where(car => car.Year == 2010);

            var _orderedCards = myCars.OrderByDescending(car => car.Year);

            foreach (var car in _orderedCards)
            {
                Console.WriteLine("{0} {1} - {2}", car.Make, car.Model, car.Year);
            }

            var sum = myCars.Sum(car => car.StickerPrice);
            Console.WriteLine("Total value of inventory: {0:C}", sum);
